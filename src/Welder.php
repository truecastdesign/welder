...
if($F->validate('name=name email=email phone=clean message=required') and $F->spam('akismet="name,email,content" nourls=true captcha=true')) # valid
{
				...
				foreach ($fields as $field) {
					if(isset($_POST[$field])) {
						...
						if($this->tooManyConsonants($_POST[$field])) {
							trigger_error("Spam type: Too Many Consonants Together!", 512);
							$result = false;
						}	

						if($this->keywordSearch($_POST[$field])) {
							trigger_error("Spam type: Keyword Search!", 512);
							$result = false;
						}	
					}	
				}
